...
function App() {
  const { toggleAddbar, settoggleAddbar, setSearchTerm, toggleEditbar} = useStore();
  ...
  return (
    <div>
      <NavBar />
      <div className={`bg-light bg-opacity-75 p-4 rounded my-5 mx-auto shadow res-w ${toggleAddbar || toggleEditbar ? 'd-none': ''}`}>
        ...
        <div className="font-weight-bold text-dark d-flex">
          ...
          <span className="material-symbols-outlined text-danger">radio_button_checked</span>
          <p className="me-1">High</p>
          <span className="material-symbols-outlined text-warning">radio_button_checked</span>
          <p className="me-1">Medium</p>
          <span className="material-symbols-outlined text-success">radio_button_checked</span>
          <p className="me-1">Low</p>
        </div>
        <TaskList />
        <button className="w-100 border rounded p-2" onClick={handleToggleAddbar}>
          New Task
        </button>
      </div>
